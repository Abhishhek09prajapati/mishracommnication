...
fetch('https://api.npoint.io/ecccc07e4a1fcb159b26')
    .then(res => res.json())
    .then(data => {
        data.BUD.name.forEach((item, index) => {
           ...
           itemmrp.innerHTML =` MRP : ${data.BUD.mrp[index]}`
           itmesrate.innerHTML =` Rate : ${data.BUD.rate[index]}`
            
            var image = document.createElement('img');
            image.classList.add("stockimg")
            image.src = data.BUD.image[index];


            nameitems.innerHTML = data.BUD.name[index];
