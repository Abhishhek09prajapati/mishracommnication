...
fetch('https://api.npoint.io/ecccc07e4a1fcb159b26')
    .then(res => res.json())
    .then(data => {
        data.Battery.name.forEach((item, index) => {
           ...
           itemmrp.innerHTML =` MRP : ${data.Battery.mrp[index]}`
           itmesrate.innerHTML =` Rate : ${data.Battery.rate[index]}`
            
            var image = document.createElement('img');
            image.classList.add("stockimg")
            image.src = data.Battery.image[index];


            nameitems.innerHTML = data.Battery.name[index];
